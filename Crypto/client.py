...
class CryptoClient:
    ...
    def buy_coin(self):
        try:
            coin = self.coin_entry.get().strip().upper()  # FIX: Standardize abbreviation format
    ...
    def sell_coin(self):
        try:
            ...
            # Get all user's transactions for this coin
            ...
            # For simplicity, just find a matching transaction or the first transaction with enough quantity
    ...
    def view_coins(self):
        self.cursor.execute('SELECT name, abbreviation, opening_price FROM crypto')
